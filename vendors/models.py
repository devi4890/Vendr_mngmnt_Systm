...
class Vendor(models.Model):                    #Data Validation used in fields
...
class PurchaseOrder(models.Model):
    ...
    status = models.CharField(max_length=20,choices=(('pending', 'Pending'),('completed', 'Completed'),('canceled', 'Canceled'), ('custom', 'Custom Status'), ),default='pending')
...
def update_vendor_performance(sender, instance, **kwargs):
    ...
    if completed_orders.exists() and completed_orders.count() > 0:
     on_time_delivery_rate = on_time_deliveries.count() / completed_orders.count()
    else:
